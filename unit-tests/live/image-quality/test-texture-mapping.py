...
def is_color_close(actual_rgb, expected_rgb):
    return all(abs(int(a) - int(e)) <= COLOR_TOLERANCE for a, e in zip(actual_rgb, expected_rgb))
...
try:
    ...
    for i in range(NUM_FRAMES):
        ...
        for idx, (x, y) in enumerate(centers):
            ...
            if invalid_values.size > sample_area.size * 0.2:
