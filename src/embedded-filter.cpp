...
namespace librealsense {
    ...
    void embedded_decimation_filter::set(rs2_embedded_filter embedded_filter_type, std::vector<uint8_t> params)
    {
        if (embedded_filter_type != RS2_EMBEDDED_FILTER_DECIMATION)
        {
    ...
    std::vector<uint8_t> embedded_decimation_filter::get(rs2_embedded_filter embedded_filter_type)
    {
        if (embedded_filter_type != RS2_EMBEDDED_FILTER_DECIMATION)
        {
            throw std::invalid_argument("Decimation filter only supports RS2_EMBEDDED_FILTER_DECIMATION type");
    ...
    bool embedded_decimation_filter::supports(rs2_embedded_filter embedded_filter_type) const
    {
        return embedded_filter_type == RS2_EMBEDDED_FILTER_DECIMATION;
    }
    ...
    void embedded_temporal_filter::set(rs2_embedded_filter embedded_filter_type, std::vector<uint8_t> params)
    {
        if (embedded_filter_type != RS2_EMBEDDED_FILTER_TEMPORAL)
        {
    ...
    std::vector<uint8_t> embedded_temporal_filter::get(rs2_embedded_filter embedded_filter_type)
    {
        if (embedded_filter_type != RS2_EMBEDDED_FILTER_TEMPORAL)
        {
            throw std::invalid_argument("Temporal filter only supports RS2_EMBEDDED_FILTER_TEMPORAL type");
    ...
    bool embedded_temporal_filter::supports(rs2_embedded_filter embedded_filter_type) const
    {
        return embedded_filter_type == RS2_EMBEDDED_FILTER_TEMPORAL;
    }
