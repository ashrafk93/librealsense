...
namespace librealsense {
    ...
    void rs_dds_embedded_decimation_filter::add_option(std::shared_ptr< realdds::dds_option > option)
    {
        ...
        else if (option->get_name() == "Toggle")
        {
    ...
    void rs_dds_embedded_decimation_filter::validate_one_option(rsutils::json opt_j)
    {
    ...
    void rs_dds_embedded_decimation_filter::validate_filter_options(rsutils::json options_j)
    {
        ...
        if (options_j.size() > 2) {
            throw std::invalid_argument("No more than 2 parameters are expected for Decimation filter (enabled + magnitude)");
        }
